...
const Connection = {
  ...
  async toggleConnection(req, res) {
    ...
    //check if blocked
    relationship_model.findOne(
      { follower: followee, followee: follower },
      function (err, relationReverse) {
          ...
          relationship_model.findOne(
            ...
            function (err, relation) {
              if (relation == null) {
                ...
                //request to follow
                let newConnection = new relationship_model({
    ...
    relationship_model.findOne(
      ...
      function (err, relation) {
        ...
        switch (relation.status) {
          case "BLOCKED": {
            relationship_model.deleteOne({ _id: relation._id }, (err) => {
              if (err) {
                return res.status(500).send({ err });
              }
  ...
  getPending(req, res) {
    ...
    var uid = decoded.user.uid;
    relationship_model.find({ follower: uid, status: "PENDING" }, function (
      err,
      relations
    ) {
      if (err) {
  ...
  async acceptRequest(req, res) {
    ...
    if (response) {
      //accept request
      relationship_model.findOneAndUpdate(
  ...
  async deleteRelation(req, res) {
    ...
    const response = await relationship_model
      .deleteOne({
        $or: [

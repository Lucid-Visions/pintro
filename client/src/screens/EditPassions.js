...
const EditPassions = ({ route }) => {
   ...
   */
  const update = () => {
    if (state.selectedTags.length >= 6) {
      const result = updateRequest({ tags: state.selectedTags });
  ...
  const tagsItems = TagPassions.map((item, index) => {
      ...
      <SafeAreaView style={styles.container1}>
        <View style={styles.container}>
          ...
          <View>
            ...
            <Text style={styles.subTitle}>Choose your passion tags (6 minimum)</Text>
          </View>
